...
cmd_line_t lgn_cmd_table[] = {
    ...
    {(const int8_t*)"motor",	shell_motor,	    (const int8_t*)"control stepper"},
...
int32_t shell_flash(uint8_t* argv) {
    ...
    case 'w': {
        uint32_t pbuf = 100;
        flash_write(APP_FLASH_FIRMWARE_START_ADDR + 3, (uint8_t *)&pbuf, sizeof(uint32_t)) ;
        LOGIN_PRINT("flash write byte... completed\n");
    }
        break;

    case 'd': {
        LOGIN_PRINT("flash read byte...:\n");
        uint32_t pbuf = 0;
        flash_read(APP_FLASH_FIRMWARE_INFO_SECTOR_1 + 3, (uint8_t *)&pbuf, sizeof(uint32_t));
        LOGIN_PRINT("flash read...: %d \n", pbuf);
    }
        break;
...
static int32_t shell_motor(uint8_t* argv) {
    ...
    uint8_t cmd_len = str_parser((char*)argv);

    if (cmd_len == 3) {
        uint8_t motor_number = atoi(str_parser_get_attr(1));
        char* cmd1 = str_parser_get_attr(2);

        switch (motor_number) {
        case 1: {
            if ((strcmp((const char* )cmd1, "start")) == 0) {
                xprintf("START MOTOR %d\n", motor_number);
                timer_50us_enable();
                GPIO_ResetBits(STEP_MOTOR1_EN_PORT, STEP_MOTOR1_EN_PIN);
            }
            else if ((strcmp((const char* )cmd1, "stop")) == 0) {
                xprintf("STOP MOTOR %d\n", motor_number);
                timer_50us_disable();
                GPIO_ResetBits(STEP_MOTOR1_EN_PORT, STEP_MOTOR1_EN_PIN);
            }
            else if ((strcmp((const char* )cmd1, "phai")) == 0) {
                xprintf("MOTOR %d QUAY PHAI\n", motor_number);
                GPIO_SetBits(STEP_MOTOR1_DIR_PORT, STEP_MOTOR1_DIR_PIN);
            }
            else if ((strcmp((const char* )cmd1, "trai")) == 0) {
                xprintf("MOTOR %d QUAY TRAI\n", motor_number);
        ...
        uint8_t motor_number = atoi(str_parser_get_attr(1));
        char* cmd1 = str_parser_get_attr(2);
        char* cmd2 = str_parser_get_attr(3);
        uint16_t step_speed = atoi(str_parser_get_attr(4));

        switch (motor_number) {
        case 1: {
            if ((strcmp((const char* )cmd1, "set")) == 0) {
                if ((strcmp((const char* )cmd2, "speed")) == 0) {
                    STEP_MOTOR_1_SPEED = step_speed;
                }
            }
        }
            break;

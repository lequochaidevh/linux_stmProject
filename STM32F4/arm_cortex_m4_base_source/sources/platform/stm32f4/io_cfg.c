...
uint8_t io_button_mode_read() {
    uint8_t value = GPIO_ReadInputDataBit(BUTTON_MODE_IO_PORT,BUTTON_MODE_IO_PIN);
    uint8_t ret = (value == 0)?1:0;
    return  ret;
}
...
void step_motor1_init() {
    GPIO_InitTypeDef GPIO_InitStructure;

    RCC_AHB1PeriphClockCmd(STEP_MOTOR1_EN_CLOCK | STEP_MOTOR1_DIR_CLOCK | STEP_MOTOR1_PULSE_CLOCK, ENABLE);

    GPIO_InitStructure.GPIO_Pin   = STEP_MOTOR1_EN_PIN | STEP_MOTOR1_DIR_PIN | STEP_MOTOR1_PULSE_PIN;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_High_Speed;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_Init(STEP_MOTOR1_EN_PORT, &GPIO_InitStructure);

    GPIO_Init(STEP_MOTOR1_DIR_PORT, &GPIO_InitStructure);

    GPIO_Init(STEP_MOTOR1_PULSE_PORT, &GPIO_InitStructure);
}

void step_motor2_init() {
    GPIO_InitTypeDef GPIO_InitStructure;

    RCC_AHB1PeriphClockCmd(STEP_MOTOR2_EN_CLOCK | STEP_MOTOR2_DIR_CLOCK | STEP_MOTOR2_PULSE_CLOCK, ENABLE);

    GPIO_InitStructure.GPIO_Pin   = STEP_MOTOR2_EN_PIN | STEP_MOTOR2_DIR_PIN | STEP_MOTOR2_PULSE_PIN;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_High_Speed;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_Init(STEP_MOTOR2_EN_PORT, &GPIO_InitStructure);

    GPIO_Init(STEP_MOTOR2_DIR_PORT, &GPIO_InitStructure);

    GPIO_Init(STEP_MOTOR2_PULSE_PORT, &GPIO_InitStructure);
}
...
void motor2_enable() {
    GPIO_ResetBits(STEP_MOTOR2_EN_PORT, STEP_MOTOR2_EN_PIN);
}
...
void motor2_nghich() {
    GPIO_ResetBits(STEP_MOTOR2_DIR_PORT, STEP_MOTOR2_DIR_PIN);
}
...
void timer_50us_init() {
    ...
    timer_50us.TIM_Period = 30 - 1;
    timer_50us.TIM_Prescaler = (SystemCoreClock/1000000) - 1;
...
void windboneFlash_spi_ctrl_init() {
    ...
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4;		/* toc do truyen; so bi chia la 48 MHz*/

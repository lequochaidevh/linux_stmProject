...
DSTATUS TM_FATFS_SD_SDIO_disk_initialize(void) {
	...
	xprintf("TM_FATFS_SD_SDIO_disk_initialize\n");

	NVIC_InitTypeDef NVIC_InitStructure;
	
	/* Detect pin */
#if FATFS_USE_DETECT_PIN > 0
	xprintf("if FATFS_USE_DETECT_PIN > 0\n");
	...
	/* Write protect pin */
#if FATFS_USE_WRITEPROTECT_PIN > 0
	xprintf("if FATFS_USE_WRITEPROTECT_PIN > 0\n");
...
DSTATUS TM_FATFS_SD_SDIO_disk_status(void) {
	...
	xprintf("TM_FATFS_SD_SDIO_disk_status\n");

	if (SD_Detect() != SD_PRESENT) {
		xprintf("SDIO_disk not init");
		return STA_NOINIT;
	}
...
SD_Error SD_Init (void)
{
	...
	if (errorstatus != SD_OK) {
		xprintf ("SD_PowerON failed\r\n");
		/*!< CMD Response TimeOut (wait for CMDSENT flag) */
		return (errorstatus);
	}

	xprintf("SD_PowerON OK\r\n");

	errorstatus = SD_InitializeCards ();

	if (errorstatus != SD_OK) {
		xprintf("SD_InitializeCards failed\r\n");
		/*!< CMD Response TimeOut (wait for CMDSENT flag) */
		return (errorstatus);
	}

	xprintf("SD_InitializeCards OK\r\n");
	...
	if (errorstatus == SD_OK) {
		/*----------------- Select Card --------------------------------*/
		xprintf("SD_GetCardInfo OK\r\n");
		errorstatus = SD_SelectDeselect ((uint32_t) (SDCardInfo.RCA << 16));
	}
	else {
		xprintf("SD_SelectDeselect failed\r\n");
	}

	if (errorstatus == SD_OK) {
		xprintf("SD_SelectDeselect OK\r\n");
		...
		xprintf("SD_EnableWideBusOperation failed\r\n");
	}

	if (errorstatus == SD_OK) {
		xprintf("SD_EnableWideBusOperation OK\r\n");
	}

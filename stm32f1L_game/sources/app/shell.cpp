...
int32_t shell_acc(uint8_t* argv);
...
const cmd_line_t lgn_cmd_table[] = {
	...
	{(const int8_t*)"acc",		shell_acc,			(const int8_t*)"air conditioner controller"},
...
int32_t shell_acc(uint8_t* argv) {
	...
	case 't': {
		uint8_t len = str_parser((char*)argv);
		if (len >= 4) {
			char* T1 = str_parser_get_attr(2);
			char* T2 = str_parser_get_attr(3);
			LOGIN_PRINT("T1: %s *C\n", T1);
			LOGIN_PRINT("T2: %s *C\n", T2);
			table_string_relay_pop_indoor.set_real(String("sIPMSt1"), static_cast<int>(String(T1).toInt()));
			table_string_relay_pop_indoor.set_real(String("sIPMSt2"), static_cast<int>(String(T2).toInt()));
		}
	}
		break;

...
//	GPIO_InitStructure.GPIO_Pin = FAN_PWM_1_IO_PIN | FAN_PWM_2_IO_PIN | FAN_PWM_3_IO_PIN | FAN_PWM_4_IO_PIN;
//	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
//	GPIO_Init(FAN_PWM_IO_PORT, &GPIO_InitStructure);
...
void io_pwm_timer3_chanel_enable(uint8_t chanel) {
	switch (chanel) {
	case 0:
		TIM_OC1PreloadConfig(TIM3, TIM_OCPreload_Enable);
		break;
...
void io_pwm_timer3_chanel_disable(uint8_t chanel) {
	switch (chanel) {
	case 0:
...
void io_pwm_timer3_set_duty_cycle(uint8_t chanel, uint16_t duty_cycle) {
	switch (chanel) {
	case 0:
	...
	case 1:
		TIM3->CCR2 = (65535 / 100) * duty_cycle;
		break;

	case 2:
		TIM3->CCR3 = (65535 / 100) * duty_cycle;
		break;

	case 3:
		TIM3->CCR4 = (65535 / 100) * duty_cycle;
		break;
...
//	/* Enable USART clock */
//	RCC_AHBPeriphClockCmd(USART_RS485_CLK, ENABLE);
